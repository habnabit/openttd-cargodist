...
uint32 ReserveConsist(Station *st, Vehicle *u, StationID next_station)
{
	...
	if (_settings_game.order.improved_load && (u->current_order.GetLoadType() & OLFB_FULL_LOAD)) {
		...
		for (Vehicle *v = u; v != NULL; v = v->Next()) {
			...
			if (cap > 0) {
				/* We could count the cargo in one round and then assign the
				 * quantities in another round over all vehicles. This would
				 * lead to an equal distribution of cargo over all vehicles
				 * in the consist. I don't think it's worth it, though.
				 */
				if (st->goods[v->cargo_type].cargo.MoveTo(&v->cargo, cap, next_station, true) > 0) {
...
static uint32 LoadUnloadVehicle(Vehicle *front, uint32 cargos_reserved)
{
	...
	cargos_reserved |= ReserveConsist(st, front, next_station);

	/* We have not waited enough time till the next round of loading/unloading */
	if (front->load_unload_ticks != 0) return cargos_reserved;
	...
	for (Vehicle *v = front; v != NULL; v = v->Next()) {
		...
		if (front->current_order.IsRefit() && artic_part == 1 && IsArticulatedVehicleEmpty(v) &&
			...
			do {
				consist_capleft[w->cargo_type] += w->cargo_cap;
				if (do_reserve && st->goods[w->cargo_type].cargo.MoveTo(&w->cargo, w->cargo_cap, next_station, true) > 0) {
		...
		if (cap_left > 0) {
			...
			if (v->cargo.ReservedCount() > 0) {
				...
				loaded = ge->cargo.MoveTo(&v->cargo, cap_left, next_station);
				consist_capleft[v->cargo_type] -= loaded;
			}
